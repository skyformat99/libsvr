...
bool _iocp_tcp_socket_bind_iocp_port(struct iocp_tcp_socket* socket)
{
...
void* _iocp_tcp_manager_alloc_memory(struct iocp_tcp_manager* mgr, size_t buffer_size)
{
    HMEMORYUNIT unit;
...
void _iocp_tcp_manager_free_memory(struct iocp_tcp_manager* mgr, void* mem, size_t buffer_size)
{
    HRBNODE memory_node = rb_tree_find_int(mgr->memory_mgr, buffer_size);

    HMEMORYUNIT check_unit = *(struct mem_unit**)((unsigned char*)mem - sizeof(void*));
...
struct iocp_tcp_socket* _iocp_tcp_manager_alloc_socket(struct iocp_tcp_manager* mgr, size_t recv_buf_size, size_t send_buf_size)
{
    struct iocp_tcp_socket* socket = 0;
    ...
    if (socket)
    {
        ...
        if (!socket->recv_loop_cache)
        {
            ...
            SOCKET_LOCK;
            socket->recv_loop_cache = create_loop_cache(recv_buf_size, (char*)_iocp_tcp_manager_alloc_memory(mgr, recv_buf_size));
            socket->send_loop_cache = create_loop_cache(send_buf_size, (char*)_iocp_tcp_manager_alloc_memory(mgr, send_buf_size));
            SOCKET_UNLOCK;
        }
        else
        {
...
void _iocp_tcp_manager_free_socket(struct iocp_tcp_manager* mgr, struct iocp_tcp_socket* socket)
{
    SOCKET_LOCK;
...
void _push_data_event(struct iocp_tcp_socket* socket, size_t data_len)
{
...
void _push_establish_event(struct iocp_tcp_listener* listener, struct iocp_tcp_socket* socket)
{
    struct net_event* evt;
    size_t evt_len = sizeof(struct net_event);
    EVENT_LOCK;
...
void _push_system_error_event(struct iocp_tcp_socket* socket, int err_code)
{
    struct net_event* evt;
    size_t evt_len = sizeof(struct net_event);
    EVENT_LOCK;
...
void _push_module_error_event(struct iocp_tcp_socket* socket, int err_code)
{
    struct net_event* evt;
    size_t evt_len = sizeof(struct net_event);
    EVENT_LOCK;
...
void _push_terminate_event(struct iocp_tcp_socket* socket)
{
    struct net_event* evt;
    size_t evt_len = sizeof(struct net_event);
    EVENT_LOCK;
...
void _push_connect_fail_event(struct iocp_tcp_socket* socket, int err_code)
{
    struct net_event* evt;
    size_t evt_len = sizeof(struct net_event);
    EVENT_LOCK;
...
void _push_recv_active_event(struct iocp_tcp_socket* socket)
{
    if (socket->state == SOCKET_STATE_ESTABLISH)
    {
        struct net_event* evt;
...
void _iocp_tcp_socket_close(struct iocp_tcp_socket* socket, int error)
{
...
bool _iocp_tcp_socket_post_recv_req(struct iocp_tcp_socket* socket)
{
...
bool _iocp_tcp_socket_post_recv(struct iocp_tcp_socket* socket)
{
    DWORD byte_received;
    DWORD flags = 0;
...
void _iocp_tcp_socket_on_recv(struct iocp_tcp_socket* socket, BOOL ret, DWORD trans_byte)
{
    ++socket->recv_ack;
    if (!ret)
    {
...
bool _iocp_tcp_socket_post_send_req(struct iocp_tcp_socket* socket)
{
...
bool _iocp_tcp_socket_post_send(struct iocp_tcp_socket* socket)
{
...
void _iocp_tcp_socket_on_send(struct iocp_tcp_socket* socket, BOOL ret, DWORD trans_byte)
{
    LONG old_data;
...
bool _iocp_tcp_socket_post_connect_req(struct iocp_tcp_socket* socket, BOOL reuse_addr)
{
    ZeroMemory(&socket->iocp_recv_data.over_lapped, sizeof(socket->iocp_recv_data.over_lapped));
...
void _iocp_tcp_socket_connect_ex(struct iocp_tcp_socket* socket_ptr, BOOL reuse_addr)
{
...
void _iocp_tcp_socket_on_connect(struct iocp_tcp_socket* socket, BOOL ret)
{
    struct sockaddr_in addr = {0};
...
void _iocp_tcp_socket_on_timer_close(struct iocp_tcp_socket* socket)
{
    switch (socket->state)
    {
...
void _mod_timer_close(struct iocp_tcp_socket* socket, int elapse)
{
...
void _mod_timer_send(struct iocp_tcp_socket* socket, int elapse)
{
    if (socket->timer_send)
    {
...
void _iocp_tcp_socket_on_timer_send(struct iocp_tcp_socket* socket)
{
    if (socket->state == SOCKET_STATE_ESTABLISH)
    {
        if (socket->data_delay_send > 0)
...
//////////////////////////////////////////////////////////////////////////
bool _iocp_tcp_listener_bind_iocp_port(struct iocp_tcp_listener* listener)
{
...
bool _iocp_tcp_listener_post_accept_ex(struct iocp_tcp_listener* listener, struct iocp_listen_data* iocp_listen_data_ptr)
{
    DWORD bytes = 0;
...
void _iocp_tcp_listener_on_accept(struct iocp_tcp_listener* listener, BOOL ret, struct iocp_listen_data* iocp_listen_data_ptr)
{
    struct iocp_tcp_socket* socket;
...
void iocp_tcp_close_listener(struct iocp_tcp_listener* listener)
{
    int i;
...
bool _iocp_tcp_listener_listen(struct iocp_tcp_listener* listener, int max_accept_ex_num, const char* ip, UINT16 port, bool bReUseAddr)
{
    int i;
    struct sockaddr_in addr={0};
...
bool _proc_net_event(struct iocp_tcp_manager* mgr)
{
    struct net_event* evt;
    size_t evt_len = sizeof(struct net_event);
    ...
    case NET_EVENT_DATA:
        {
            ...
            while (socket->data_has_recv)
            {
                ...
                {
                    pkg_len = socket->pkg_parser(data_ptr, socket->data_has_recv);
                }
                else
                {
...
{
    struct iocp_tcp_manager* mgr = (struct iocp_tcp_manager*)param;

    struct iocp_data* iocp_data_ptr;
    struct iocp_tcp_socket* iocp_tcp_socket_ptr;
    BOOL ret;
...
bool _start_iocp_thread(struct iocp_tcp_manager* mgr)
{
    int i;
...
void _stop_iocp_thread(struct iocp_tcp_manager* mgr)
{
    int i;
    ULONG_PTR ptr = 0;
    for (i = 0; i < mgr->work_thread_num; i++)
...
{
    struct iocp_tcp_socket* socket = (struct iocp_tcp_socket*)timer_get_data(timer);
...
bool _set_wsa_function(struct iocp_tcp_manager* mgr)
{
    DWORD bytes;
...
void destroy_iocp_tcp(struct iocp_tcp_manager* mgr)
{
    ...
    if (mgr->memory_mgr)
    {
        ...
        while (memory_node)
        {
            //_destroy_memory_pool((struct mem_pool*)rb_node_value(memory_node));
            destroy_memory_unit((HMEMORYUNIT)rb_node_value(memory_node));
            memory_node = rb_next(memory_node);
        }
        destroy_rb_tree(mgr->memory_mgr);
        mgr->memory_mgr = 0;
    }
...
struct iocp_tcp_manager* create_iocp_tcp(pfn_on_establish func_on_establish, pfn_on_terminate func_on_terminate, 
    ...
    int max_socket_num, int max_io_thread_num, int max_accept_ex_num)
{
    ...
    struct iocp_tcp_socket** arry_socket_ptr = 0;

    struct iocp_tcp_manager* mgr = (struct iocp_tcp_manager*)malloc(sizeof(struct iocp_tcp_manager));
    ...
    arry_socket_ptr = (struct iocp_tcp_socket**)malloc(sizeof(struct iocp_tcp_socket*)*max_socket_num);
    ...
    mgr->memory_mgr = create_rb_tree(0);
    if (!mgr->memory_mgr)
    {
        goto ERROR_DEAL;
    }
...
struct iocp_tcp_socket* iocp_tcp_connect(struct iocp_tcp_manager* mgr,
...
{
    struct iocp_tcp_socket* socket = _iocp_tcp_manager_alloc_socket(mgr, recv_buf_size, send_buf_size);
...
struct iocp_tcp_listener* iocp_tcp_listen(struct iocp_tcp_manager* mgr,
    const char* ip, unsigned short port, int recv_buf_size, int send_buf_size, 
    pfn_parse_packet func, bool reuse_addr)
{
    struct iocp_tcp_listener* listener = (struct iocp_tcp_listener*)malloc(sizeof(struct iocp_tcp_listener));
...
bool iocp_tcp_send(struct iocp_tcp_socket* socket, const char* data, size_t len)
{
    if (len <= 0)
    {
        return true;
...
void iocp_tcp_close_session(struct iocp_tcp_socket* socket)
{
    _iocp_tcp_socket_close(socket, ERROR_NONE);
}

bool iocp_tcp_run(struct iocp_tcp_manager* mgr, unsigned int run_time)
{
...
SOCKET iocp_tcp_session_socket(struct iocp_tcp_socket* socket)
{
    return socket->socket;
}

SOCKET iocp_tcp_listener_socket(struct iocp_tcp_listener* listener)
{
    return listener->socket;
}

void iocp_tcp_set_listener_data(struct iocp_tcp_listener* listener, void* user_data)
{
    listener->user_data = user_data;
}

void iocp_tcp_set_session_data(struct iocp_tcp_socket* socket, void* user_data)
{
    socket->user_data = user_data;
}

void* iocp_tcp_get_listener_data(struct iocp_tcp_listener* listener)
{
    return listener->user_data;
}

void* iocp_tcp_get_session_data(struct iocp_tcp_socket* socket)
{
    return socket->user_data;
}

unsigned int iocp_tcp_get_peer_ip(struct iocp_tcp_socket* socket)
{
    return socket->peer_ip;
}

unsigned short iocp_tcp_get_peer_port(struct iocp_tcp_socket* socket)
{
    return socket->peer_port;
}

unsigned int iocp_tcp_get_local_ip(struct iocp_tcp_socket* socket)
{
    return socket->local_ip;
}

unsigned short iocp_tcp_get_local_port(struct iocp_tcp_socket* socket)
{
    return socket->local_port;
}

size_t iocp_tcp_get_send_free_size(struct iocp_tcp_socket* socket)
{
    return loop_cache_free_size(socket->send_loop_cache);
}

void iocp_tcp_set_send_control(struct iocp_tcp_socket* socket, int pkg_size, int delay_time)
{
