...
void destroy_quick_tree(HRBTREE tree)
{
    ...
    while (node)
    {
        default_memory_pool_free(rb_node_key_user(node));
        default_memory_pool_free(rb_node_value(node));
...
HRBTREE quick_tree(HRBTREE tree, size_t elapse)
{
    ...
    void** value_group = (void**)default_memory_pool_alloc(value_group_count*sizeof(void*));
    ...
    while (node)
    {
        if (key_group)
        {
            ...
            if (key - key_group->key_end <= elapse)
            {
                ...
                while (key_group->key_end < key)
                {
                    if (group_count >= value_group_count)
                    {
                        ...
                        tmp = (void**)default_memory_pool_alloc(value_group_count*sizeof(void*));
                        memcpy(tmp, value_group, group_count*sizeof(void*));
                        default_memory_pool_free(value_group);
                        value_group = tmp;
                    }
            ...
            {
                void** real_value_group = (void**)default_memory_pool_alloc(group_count*sizeof(void*));
                memcpy(real_value_group, value_group, group_count*sizeof(void*));
                rb_tree_insert_user(new_tree, key_group, real_value_group);

                key_group = (int_key_group*)default_memory_pool_alloc(sizeof(int_key_group));
...
void add_col_info(HRBTREE tree, const char* col_var_name, size_t col_var_offset, col_data_type col_var_type)
{
    ...
    info = (col_var_info*)default_memory_pool_alloc(sizeof(col_var_info));
    ...
    {
        default_memory_pool_free(rb_node_value(exist_node));
