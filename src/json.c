...
struct json_struct
{
    ...
    struct json_struct*     stack;
};

HMEMORYUNIT _get_json_node_unit(void)
{
    static HMEMORYUNIT json_node_unit = 0;

    if (json_node_unit)
    {
        return json_node_unit;
    }

    json_node_unit = create_memory_unit(sizeof(struct json_node));

    return json_node_unit;
}

HMEMORYUNIT _get_json_struct_unit(void)
{
    static HMEMORYUNIT json_struct_unit = 0;

    if (json_struct_unit)
    {
        return json_struct_unit;
    }

    json_struct_unit = create_memory_unit(sizeof(struct json_struct));
...
struct json_struct* _json_create_object(void)
{
    struct json_struct* json_struct_ptr = (struct json_struct*)memory_unit_alloc(_get_json_struct_unit(), 4*1024);
...
struct json_struct* _json_create_array(void)
{
    struct json_struct* json_struct_ptr = (struct json_struct*)memory_unit_alloc(_get_json_struct_unit(), 4*1024);
    json_struct_ptr->head = 0;
    json_struct_ptr->tail = 0;
...
struct json_struct* create_json(enum e_json_value_type type)
{
    switch (type)
    {
    case json_object:
...
void destroy_json(struct json_struct* json_struct_ptr)
{
    struct json_node* node_ptr = 0;
    struct json_node* node_stack_head = 0;
    struct json_node* free_node = 0;
    ...
    while (node_ptr)
    {
        if (node_ptr->key)
        {
            default_memory_pool_free(node_ptr->key);
        }
        ...
        case json_null:
            {
                ...
                memory_unit_free(_get_json_node_unit(), free_node);
            }
            break;
        case json_string:
            {
                default_memory_pool_free(node_ptr->value.str);
        ...
        else
        {
            for (;;)
            {
                ...
                if (node_ptr)
                {
                    ...
                    {
                        node_ptr = node_ptr->next;
                        memory_unit_free(_get_json_struct_unit(), free_node->value.struct_ptr);
                        memory_unit_free(_get_json_node_unit(), free_node);
                        break;
                    }
                    else
    ...
    memory_unit_free(_get_json_struct_unit(), json_struct_ptr);
}

void _json_struct_add_node(struct json_struct* json_struct_ptr, struct json_node* json_node_ptr)
{
...
struct json_node* json_add_float(struct json_struct* json_struct_ptr, double number, const char* key, size_t key_length)
{
    struct json_node* node = (struct json_node*)memory_unit_alloc(_get_json_node_unit(), 4*1024);
    ...
    case json_object:
        {
            if (!key)
            {
                memory_unit_free(_get_json_node_unit(), node);
                return 0;
            }
            node->key_length = key_length;
            node->key = (char*)default_memory_pool_alloc(node->key_length+1);
...
struct json_node* json_add_integer(struct json_struct* json_struct_ptr, long long number, const char* key, size_t key_length)
{
    struct json_node* node = (struct json_node*)memory_unit_alloc(_get_json_node_unit(), 4*1024);
    ...
    case json_object:
        {
            ...
            node->key = (char*)default_memory_pool_alloc(key_length+1);
...
struct json_node* json_add_string(struct json_struct* json_struct_ptr, const char* string, size_t string_length, const char* key, size_t key_length)
{
    struct json_node* node = (struct json_node*)memory_unit_alloc(_get_json_node_unit(), 4*1024);
    ...
    node->value.str = (char*)default_memory_pool_alloc(string_length+1);
...
struct json_node* json_add_true(struct json_struct* json_struct_ptr, const char* key, size_t key_length)
{
    struct json_node* node = (struct json_node*)memory_unit_alloc(_get_json_node_unit(), 4*1024);
...
struct json_node* json_add_false(struct json_struct* json_struct_ptr, const char* key, size_t key_length)
{
    struct json_node* node = (struct json_node*)memory_unit_alloc(_get_json_node_unit(), 4*1024);
...
struct json_node* json_add_null(struct json_struct* json_struct_ptr, const char* key, size_t key_length)
{
    struct json_node* node = (struct json_node*)memory_unit_alloc(_get_json_node_unit(), 4*1024);
...
struct json_struct* json_add_struct(struct json_struct* json_struct_ptr, 
    enum e_json_value_type type, const char* key, size_t key_length)
{
    struct json_node* new_json_node = 0;

    new_json_node = (struct json_node*)memory_unit_alloc(_get_json_node_unit(), 4*1024);
    ...
    case json_object:
        {
            if (!key)
            {
                memory_unit_free(_get_json_struct_unit(), new_json_node->value.struct_ptr);
                memory_unit_free(_get_json_node_unit(), new_json_node);
                return 0;
            }
            new_json_node->key = (char*)default_memory_pool_alloc(key_length+1);
...
void json_to_string_ex(struct json_struct* json_struct_ptr, json_string_append f, void* ud)
{
    char tmp_buf[64];
    size_t tmp_length = 0;
    struct json_node* node_ptr = json_struct_ptr->head;
    struct json_node* node_stack_head = 0;
...
size_t json_to_string(struct json_struct* json_struct_ptr, char* json_str_ptr, size_t json_str_size)
{
    char tmp_buf[64];
    size_t tmp_length = 0;
    struct json_node* node_ptr = json_struct_ptr->head;
...
bool step_key_value(const char** ptr, struct json_struct* json_struct_ptr, const char** key_ptr, size_t* key_length)
{
    bool is_double;
...
bool step_value(const char** ptr, struct json_struct* json_struct_ptr)
{
    bool is_double;
    const char* value_ptr = 0;
    size_t value_length = 0;
...
struct json_struct* string_to_json(const char* str, char* err_info, size_t err_info_len)
{
    struct json_struct* current_json_struct = 0;
    const char* current_ptr = str;
    struct json_struct* json_struct_stack_head = 0;

    struct json_struct* json_struct_root = 0;
...
struct json_node* json_struct_first_node( struct json_struct* json_struct )
{
    return json_struct->head;
}

struct json_node* json_struct_next_node( struct json_node* json_node )
{
    return json_node->next;
}

const char* json_key( struct json_node* json_node )
{
    return json_node->key;
}

bool json_value_integer( struct json_node* json_node, long long* value )
{
...
bool json_value_float( struct json_node* json_node, double* value )
{
...
bool json_value_string( struct json_node* json_node, const char** value, size_t* length )
{
...
bool json_value_true( struct json_node* json_node )
{
...
bool json_value_false( struct json_node* json_node )
{
...
bool json_value_null( struct json_node* json_node )
{
...
struct json_struct* json_value_struct( struct json_node* json_node )
{
...
enum e_json_value_type json_value_type( struct json_node* json_node )
{
    return json_node->type;
}

enum e_json_value_type json_struct_type( struct json_struct* json_struct )
{
